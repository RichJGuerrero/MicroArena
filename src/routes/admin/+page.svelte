<script lang="ts">
	...
	async function banTarget(userId: string, matchId: string | null) {
		error = null;
		...
		try {
			const res = await fetch(`/api/admin/users/${encodeURIComponent(userId)}/ban`, {
				method: 'POST',
				headers: { 'content-type': 'application/json' },
				body: JSON.stringify({ reason: 'Confirmed cheating', matchId })
			});
			const data = await res.json();
	...
	{:else}
		...
		<div class="grid" style="grid-template-columns: 1.1fr 1.4fr; gap: var(--space-lg); align-items:start;">
			<div class="card">
				...
				<div class="card-header">
					<h2>Recent Audit</h2>
					<p class="text-secondary">Admin/ref overrides (V0).</p>
				</div>
				<div class="flow tight">
					<div style="display:flex; justify-content:flex-end;">
						<button class="btn ghost sm" on:click={() => loadAudit(100)} disabled={loading}>Refresh</button>
					</div>
					{#if audit.length === 0}
						<div class="text-muted">No audit events yet.</div>
					{:else}
						{#each audit.slice(0, 20) as a}
