...
import { adminResolveArenaMatch, createAuditEvent, createIntegrityEvent, getArenaMatch } from '$lib/server/store';
	...
	try {
		...
		try {
			createAuditEvent({
				...
				matchId: id,
				clanId: null,
				note: `Resolved: winner ${winnerSide}${typeof scoreA === 'number' && typeof scoreB === 'number' ? ` (${scoreA}-${scoreB})` : ''}`
			});
		} catch {
			// ignore
		}
