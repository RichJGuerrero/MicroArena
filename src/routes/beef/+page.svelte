<script lang="ts">
	...
	function getStatusClass(status: string): string {
		switch (status) {
			...
			case 'PENDING': case 'ACCEPTED': return 'upcoming';
						...
						<div class="match-card card">
							...
							{#if beef.status === 'PENDING' && $currentUser?.clanId === beef.challengedClanId}
								<div class="match-actions">
									...
									<button class="btn secondary sm" on:click={() => respondToBeef(beef.id, false)}>Decline</button>
								</div>
							{/if}
						</div>
					{/each}
...
<style>
	...
	.matches-section { margin-bottom: var(--space-2xl); }
	.matches-section h2 { margin-bottom: var(--space-lg); }
	.matches-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: var(--space-md); }
	
	.match-card { padding: var(--space-lg); }
	.match-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--space-md); }
	.match-format { font-family: var(--font-mono); font-weight: 700; color: var(--text-muted); }
	
	.match-teams { display: flex; align-items: center; justify-content: space-between; gap: var(--space-md); margin-bottom: var(--space-md); }
	.team { text-align: center; }
	.team .clan-tag { display: block; font-size: 1.25rem; }
	.team .clan-name { font-size: 0.75rem; color: var(--text-muted); }
	.team.winner .clan-tag { color: var(--success); }
	.vs { font-weight: 700; color: var(--text-muted); }
	.winner-badge { display: inline-block; background: var(--success); color: #000; font-size: 0.75rem; font-weight: 700; padding: 2px 6px; border-radius: var(--radius-sm); margin-top: var(--space-xs); }
	
	.ruleset { font-size: 0.875rem; color: var(--text-secondary); margin-bottom: var(--space-md); }
	.score { font-family: var(--font-mono); font-size: 1.25rem; font-weight: 700; text-align: center; }
	
	.match-meta { display: flex; gap: var(--space-xs); margin-bottom: var(--space-md); }
	.badge { font-size: 0.6875rem; font-weight: 600; padding: 2px 6px; background: var(--bg-tertiary); border-radius: var(--radius-sm); color: var(--text-muted); }
